...
public class ExpandableAdapter extends BaseExpandableListAdapter {
	...
	public View getGroupView(final int groupPosition, boolean isExpanded,
		...
		TextView itemdescView = (TextView) convertView
				.findViewById(R.id.descriptiontxt);
		TextView priceView = (TextView) convertView
				.findViewById(R.id.descriptionitemPrice);

		numberView
				.setText(Integer.toString(User.getInstance(mContext).currentSession.currentOrder
						.getQuantityOfDishByIndex(groupPosition)));
		itemdescView
				.setText(User.getInstance(mContext).currentSession.currentOrder.mItems
						.get(groupPosition).dish.name);
		priceView
				.setText(Double.toString(User.getInstance(mContext).currentSession.currentOrder.mItems
						.get(groupPosition).dish.price));
		TextView modifierSubTitle = (TextView) convertView
				.findViewById(R.id.subTitle);
		modifierSubTitle.setVisibility(View.GONE);

		if (User.getInstance(mContext).currentSession.currentOrder.mItems
				.get(groupPosition).dish.customizable) {
			modifierSubTitle.setVisibility(View.VISIBLE);
			modifierSubTitle
					.setText(User.getInstance(mContext).currentSession.currentOrder
							.getModifierDetailsTextAtIndex(groupPosition));
		}

		ImageButton minusButton = (ImageButton) convertView
				.findViewById(R.id.imgMinus);
		ImageButton plusButton = (ImageButton) convertView
				.findViewById(R.id.imgPlus);
		minusButton.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View view) {
				if (User.getInstance(mContext).currentSession.currentOrder
						.getQuantityOfDishByIndex(groupPosition) > 0) {
				...
				User.getInstance(mContext).currentSession.currentOrder
						.decrementDishAtIndex(groupPosition);
				notifyDataSetChanged();
				if (User.getInstance(mContext).currentSession.currentOrder
						.getQuantityOfDishByIndex(groupPosition) == 0) {
					User.getInstance(mContext).currentSession.currentOrder
							.removeDishAtIndex(groupPosition);
					notifyDataSetChanged();
				}
			}
		});
		...
		plusButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				User.getInstance(mContext).currentSession.currentOrder
						.incrementDishAtIndex(groupPosition);
				notifyDataSetChanged();
				final View parent = (View) view.getParent().getParent()
						.getParent().getParent().getParent();
				TextView cornertext;
	...
	public int getGroupCount() {
		return User.getInstance(mContext).currentSession.currentOrder.mItems
				.size();
	}
